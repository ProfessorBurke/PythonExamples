...
class Student(pygame.sprite.Sprite):
    ...
    def lose_health(self, group: pygame.sprite.Group,
                    invincible_group: pygame.sprite.Group) -> None:
        self._health = 0
        self.image.set_alpha(150)
        self._invincible_timer = 150
        group.empty()
        invincible_group.add(self)
    ...
    def update(self, bricks: pygame.sprite.Group,
               group: pygame.sprite.Group, invincible_group: pygame.sprite.Group) -> None:
        self.rect.top += self._dy
        self.rect.left += self._dx
...
def main() -> None:
    ...
    invincible_group: pygame.sprite.Group = pygame.sprite.Group()
    ...
    for i in range(10):
        enemies.add(Enemy(enemy_image, 350, 230))
    ...
    while not user_quit:
        ...
        if bites:
            sam.lose_health(group, invincible_group)

        # Move, redraw, and show.
        group.update(bricks, group, invincible_group)
        invincible_group.update(bricks, group, invincible_group)
        gold.update()
        score_group.update(screen)
        enemies.update(bricks)
