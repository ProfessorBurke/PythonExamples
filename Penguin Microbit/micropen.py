...
# Libraries to interface with the microbit
...
# Set up microbit event constants.
ACCEL_EVENT: int = pygame.event.custom_type()
BUTTON_A_PRESSED: int = pygame.event.custom_type()
...
class AnimatedGroup(pygame.sprite.Group):
    """A group for any Sprite that is blitting only part of its image."""
    def draw(self, surface):
        """Draw all sprites onto the surface.
           Code modified from AbstractGroup.draw
        """
        sprites = self.sprites()
        for sprite in sprites:
            self.spritedict[sprite] = surface.blit(sprite.image, sprite.rect, sprite.source_rect)
        self.lostsprites = []
...
class Penguin(pygame.sprite.Sprite):
    ...
    def __init__(self, screen: pygame.Surface) -> None:
        ...
        super().__init__()
        self.image = pygame.image.load("penguin_sprite_sheet2.png")
        self.image = self.image.convert_alpha()
        # source_rect is the rectangle from image to use
        # rect is the location to blit
        self.source_rect = pygame.Rect(0, 0, 36, 36)
    ...
    def update(self, bricks: pygame.sprite.Group) -> None:
        ...
        if not self.on_ground(bricks):
            if self._facing == LEFT:
                index = self._index % 9 + 11
                self.source_rect = pygame.Rect(index * 36, 0, 36, 36)
                self._index += 1
            else:
                index = self._index % 9 + 2
                self.source_rect = pygame.Rect(index * 36, 0, 36, 36)
                self._index += 1
        else:
            if self._facing == LEFT:
                self.source_rect = pygame.Rect(36, 0, 36, 36)
            else:
                self.source_rect = pygame.Rect(0, 0, 36, 36)
            self._index = 0
...
def print_accelerometer_data(data:AccelerometerData):
    caption:str = str(data.x) + "," + str(data.y)
    # pygame.display.set_caption(caption)
    print(caption)
...
def post_buttonapress_event(data: ButtonState):
    #event_dict: dict = {}
    pygame.event.post(pygame.event.Event(BUTTON_A_PRESSED))
...
def main() -> None:
    ...
    penguin_group = AnimatedGroup(penguin)
    clock: pygame.time.Clock = pygame.time.Clock()


    # Find the microbit.
    with KaspersMicrobit.find_one_microbit() as microbit:
        #microbit.accelerometer.notify(print_accelerometer_data)
        microbit.accelerometer.notify(post_accelerometer_event)
        microbit.buttons.on_button_a(press=post_buttonapress_event)
