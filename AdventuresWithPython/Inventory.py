...
class Inventory:
    ...
    def to_json(self) -> dict[str, str]:
        """Return a dictionary of this object's data."""
        return self._items

    def from_json(self, json: dict[str, str]) -> None:
        """Set this object's fields from the json parameter."""
        self._items = json
...
def main() -> None:
    ...
    # Default value for initializer argument
    inv2 = Inventory()
    ...
    print("*"*80)
    print("Reading a json with items into an inventory that already has items.")
    print("Expected: knife and nest, indented.")
    print("Actual:")
    inv3: Inventory = Inventory({"one": "thing one", "two": "thing two"})
    inv3.from_json({"knife": "A nasty knife",
                     "nest": "The result of some avian hard work"})
    print(inv3)

    print("*"*80)
    print("Reading an empty json into an inventory that already has items.")
