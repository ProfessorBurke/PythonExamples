...
##        for x, y in self._cracks:
##            crack_surface = self._crack_mask.to_surface(
##                setcolor=(0, 0, 0),
##                unsetcolor=(0, 0, 0, 0)
##            )
##
##            self.image.blit(
##                crack_surface,
##                (x - crack_surface.get_width() // 2,
##                 y - crack_surface.get_height() // 2)
##            )

        center = self.rect.center
        self.image = pygame.transform.rotate(self._base_image,
                                             2 * self._hits)
    ...
    def __init__(self, image: pygame.Surface,
        ...
        crack = pygame.image.load(Path("images") / "crack.png")
        self._crack_mask = pygame.mask.from_surface(crack)
    ...
    def add_crack(self, x: int, y: int) -> bool:
        done_cracking: bool = False
        self._cracks.append((x, y))
        self._hits += 1
...
for color in brick_colors:
    brick_surf = pygame.Surface((brick_width, BRICK_HEIGHT), pygame.SRCALPHA)
...
while running:
    ...
    balls = pygame.sprite.groupcollide(ball_group, brick_group, False, False,
    ...
    if balls:
        ...
        for brick in all_bricks_hit:
            #print(count, id(brick))
            offset = (ball.rect.x - brick.rect.x, ball.rect.y - brick.rect.y)
            hit = brick.mask.overlap(ball.mask,offset)
            if hit is not None:
                destroy = brick.add_crack(*hit)
                if destroy:
                    brick.kill()
