...
from powerups11 import PowerUp, PowerupFactory, Droppable, DroppableFactory

class AnimatedWildcard(pygame.sprite.Sprite):
    """Note that the flappy bird image I downloaded is stored in separate files,
       so this animated sprite is implemented differently!"""
    def __init__(self, images: list[pygame.Surface],
                 x: int, y: int, dx: int = 8) -> None:
        super().__init__()
        self.image = images[0]
...
pygame.display.set_caption("Brickout 11.0")
clock = pygame.time.Clock()
...
# Create the animated wildcard group.
wildcard_group: pygame.sprite.Group = pygame.sprite.Group()
wildcard_images: list[Surface] = []
for i in range(1, 8):
    wildcard_images.append(pygame.image.load("images/frame-"+str(i)+".png").convert_alpha())
...
while running:
    ...
    wildcard_group.clear(screen, background)
    ...
    # Randomly create a wildcard periodically and add it to the wildcard group.
    if random.randint(1, 80) == 1:
        animated_wildcard: AnimatedWildcard = AnimatedWildcard(wildcard_images,
                 0, random.randint(100, 250))
        wildcard_group.add(animated_wildcard)
    
    #Check for wildcard collisions with droppables.
    droppables = pygame.sprite.groupcollide(droppable_group, wildcard_group, True, False,
                                       pygame.sprite.collide_mask)
