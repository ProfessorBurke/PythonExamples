...
class Paddle(pygame.sprite.Sprite):
    ...
    def _make_image(self, width: int, height: int,
                    x: int, y: int, color: tuple[int]) -> None:
        self.image = pygame.Surface((width, height))
        self.image.fill(color)
        self.rect = self.image.get_rect()
        self.rect.topleft = (x, y)
    ...
    def __init__(self, width: int, height: int,
        ...
        super().__init__()
        self._make_image(width, height, x, y, (0, 0, 255))
        self._dx = dx
    ...
    def update(self, screen: pygame.Surface) -> None:
        color: tuple[int] = self._effect_manager.get_paddle_color()
        if color is None:
            color = (0, 0, 255)
...
class PointsMultiplier(PowerUp):
    def __init__(self, image: pygame.Surface,
                 effect_manager: "EffectManager",
                 centerx: int, centery: int) -> None:
        super().__init__(image, effect_manager, centerx, centery)
...
class PaddleArtist(PowerUp):
    def __init__(self, image: pygame.Surface,
                 ...
                 centerx: int, centery: int) -> None:
        super().__init__(image, effect_manager, centerx, centery)
        self.effect_manager.change_paddle_color((random.randint(5, 255),
                                                 random.randint(5, 255),
                                                 random.randint(5, 255)))
        
    def _kill_effect(self) -> None:
        self.effect_manager.reset_paddle_color()
...
class PowerupFactory:
    ...
    def get_powerup(self, powerup_str: str, x: int, y: int) -> PowerUp | None:
...
class EffectManager:
    def __init__(self) -> None:
        ...
        self._paddle_color = None

    def change_paddle_color(self, color: tuple[int]) -> None:
        self._paddle_color = color

    def reset_paddle_color(self) -> None:
        self._paddle_color = None

    def get_paddle_color(self) -> tuple[int] | None:
        return self._paddle_color
...
while running:
    ...
    for droppable in droppables:
        ...
        power_up: PowerUp | None = powerup_factory.get_powerup(droppable.get_powerup(),
